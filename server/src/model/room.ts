...
import { MessageOutOk, MessageOutErr } from "../utils/types.ts"
...
export class Room implements Serializable<RoomDTO> {
  readonly id: string
  ...
  private constructor(
    ...
    this._log(logger, "info", "new room")
  }
  ...
  private _addUser(user: User, username: string, token: string): RoomUser {
    ...
    this._log(logger, "info", `user ${user.id} added`)
    return roomUser
  }
  ...
  >(obj: T, key: K, msg: string) {
    return obj[key](`ROOM[${this.id}]: ${msg}`)
  }
  ...
  lock() {
    this._locked = true
    this._log(logger, "info", "room locked")
  }
  unlock() {
    this._locked = false
    this._log(logger, "info", "room unlocked")
  }
  ...
  removeUser(user: User) {
    ...
    this._log(logger, "info", `user ${user.id} removed`)
    if (this._users.size === 0) {
      this._lastVisitorDate = Date.now()
      this._log(logger, "info", "room is empty")
    }
  }
  ...
  addInvitation(invitation: Invitation) {
    ...
    this._log(logger, "info", `invitation added ${invitation.id}`)
  }
  removeInvitation(invitation: Invitation) {
    this._invitations.delete(invitation.id)
    this._log(logger, "info", `invitation removed ${invitation.id}`)
  }
